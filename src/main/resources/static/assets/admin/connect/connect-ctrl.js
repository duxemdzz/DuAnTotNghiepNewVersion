app.controller("connect-ctrl", function($scope, $http) {
	$scope.items = [];
	$scope.form = {};
	$scope.cates = [];
	$scope.sortType = 'id';
	...
	$scope.initialize = function() {
		...
		$http.get(`/TechnoShop/rest/connect`).then(resp => {
	...
	$scope.create = function() {
		...
		if(item.advanceSecurity==null){
			...
			$http.post('/TechnoShop/rest/connect', item).then(resp => {
	...
	$scope.update = function() {
		...
		if(item.advanceSecurity==null){
			...
			if(item.id != null){
			$http.put(`/TechnoShop/rest/connect/${item.id}`, item).then(resp => {
			var index = $scope.items.findIndex(p => p.id == item.id);
	...
	$scope.delete = function(item) {
		...
		if (result.isConfirmed) {
				if(item.id != null){
					$http.delete(`/TechnoShop/rest/connect/${item.id}`).then(resp => {
					var index = $scope.items.findIndex(p => p.id == item.id);
