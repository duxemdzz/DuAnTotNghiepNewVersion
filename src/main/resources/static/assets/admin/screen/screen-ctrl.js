app.controller("screen-ctrl", function($scope, $http) {
	$scope.items = [];
	$scope.form = {};
	$scope.cates = [];
	$scope.sortType = 'id';
	...
	$scope.initialize = function() {
		...
		$http.get(`/TechnoShop/rest/screen`).then(resp => {
	...
	$scope.create = function() {
		...
		if(item.typeScreen==null){
			Swal.fire({
				icon: 'error',
				title: 'Vui lòng bảo mật !',
				type: 'error',
				timer:'2000'
			})
			...
			Swal.fire({
				icon: 'error',
				title: 'Vui lòng nhập kích cỡ!',
				type: 'error',
				timer:'2000'
			})
		}else if(item.sweepFrequency==null){
			Swal.fire({
				icon: 'error',
				title: 'Vui lòng nhập độ phân giải!',
				type: 'error',
				timer:'2000'
		})
		}else{
			$http.post('/TechnoShop/rest/screen', item).then(resp => {
			...
			if (index == -1) {
				...
				Swal.fire('Màn hình này đã tồn tại!', '', 'error')
			}
		}).catch(error => {
	...
	$scope.update = function() {
		...
		if(item.typeScreen==null){
			...
			if(item.id != null){
			$http.put(`/TechnoShop/rest/screen/${item.id}`, item).then(resp => {
			var index = $scope.items.findIndex(p => p.id == item.id);
	...
	$scope.delete = function(item) {
		...
		if (result.isConfirmed) {
				if(item.id != null){
					$http.delete(`/TechnoShop/rest/screen/${item.id}`).then(resp => {
					var index = $scope.items.findIndex(p => p.id == item.id);
